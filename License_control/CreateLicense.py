...
seperateKey = "ylkj8702" # 随意输入一组字符串
...
def encrypt2(text):
    ...
    if count % length != 0:
        add = length - (count % length)
    text = text + ('0'.encode() * add)  # '\0'*add 表示add个空格,即填充add个直至符合16的倍数
...
if __name__ == "__main__":
    ...
    encryptText = encryptText + seperateKey.encode() + "Valid".encode()

    encryptText = encrypt2(encryptText) # 将加密之后的密文再次加密
